...
export function montarLog(pUsuario = null, pDescricao = null, pResponse = null) {
  ...
  try {
    ...
    if (status === 400 || status === 500) {
      const chaveErro = extrairMensagemErro(resposta);
      if (!erros[chaveErro]) {
        erros[chaveErro] = 1;

        console.error('🔴 ERRO NOVO DETECTADO:');
        console.error(`[${pUsuario.usuarioBase.username}] - ${pDescricao}`);
        console.error(`Mensagem: ${chaveErro}`);
        console.error(`Status: ${status} | Tempo: ${tempo}s`);

      } else {
        erros[chaveErro]++;
      }
      console.log('[DEBUG] erro registrado:', chaveErro);
    }
  } catch (error) {
...
function extrairMensagemErro(resposta) {
  try {
    if (resposta.includes('Access violation')) {
      return 'Access violation';
    }
    return resposta.slice(0, 120); 
  } catch (e) {
